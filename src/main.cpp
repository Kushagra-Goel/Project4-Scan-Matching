...
void readPointCloud(string filename, vector<glm::vec3> &points) {
	ifstream fp_in;
	char* fname = (char*)filename.c_str();
	fp_in.open(fname);
	if (!fp_in.is_open()) {
		cout << "Error reading from file - aborting! "<< strerror(errno) << endl;
		throw;
	}
	int i = 0;
	while (fp_in.good()) {
		...
		if (!line.empty()) {
			...
			points.push_back(pt);
			if (RANDTRANSFORM) {
				transformedPoints.push_back(glm::vec3(randomTransform * glm::vec4(pt, 1)));
			}
		}
	}
}
...
void initShaders(GLuint * program) {
  ...
  void runCUDA() {
	...
	if (CPU && !GPU && !KDTREE) {
		Scan_Matching::runCPU(N_FOR_VIS, originalPoints, transformedPoints);
	}
	else {
		if (GPU && !CPU && !KDTREE) {
			Scan_Matching::runGPU(originalPoints.size(), transformedPoints.size());
